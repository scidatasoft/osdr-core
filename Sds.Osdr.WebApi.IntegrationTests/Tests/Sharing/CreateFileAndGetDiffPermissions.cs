...
namespace Sds.Osdr.WebApi.IntegrationTests.Tests.Sharing
{
    ...
    {
        public Guid JohnBlobId { get; set; }
        public Guid JohnFileId { get; }

        public Guid JaneBlobId { get; set; }
        public Guid JaneFileId { get; }

        public CreateFileAndGetDiffPermissionsFixture(OsdrWebTestHarness harness)
        {
            JohnBlobId = harness.JohnBlobStorageClient.AddResource(harness.JohnId.ToString(), "Aspirin.mol", new Dictionary<string, object>() { { "parentId", harness.JohnId } }).Result;
            JohnFileId = harness.WaitWhileRecordsFileProcessed(JohnBlobId);

            JaneBlobId = harness.JaneBlobStorageClient.AddResource(harness.JaneId.ToString(), "ringcount_0.mol", new Dictionary<string, object>() { { "parentId", harness.JaneId } }).Result;
            JaneFileId = harness.WaitWhileRecordsFileProcessed(JaneBlobId);
