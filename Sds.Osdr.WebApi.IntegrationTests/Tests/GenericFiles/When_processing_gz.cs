...
namespace Sds.Osdr.WebApi.IntegrationTests
{
    public class UploadGzFixture
    {
        ...
        {
            BlobId = harness.JohnBlobStorageClient.AddResource(harness.JohnId.ToString(), "wikiAspirin.gz", new Dictionary<string, object>() { { "parentId", harness.JohnId } }).Result;
    ...
    public class UploadGZ : OsdrWebTest, IClassFixture<UploadGzFixture>
    {
        private Guid BlobId { get; set; }
        private Guid FileId { get; set; }

        public UploadGZ(OsdrWebTestHarness fixture, ITestOutputHelper output, UploadGzFixture initFixture) : base(fixture, output)
        {
        ...
        public async Task GenericFileUpload_ValidGz_GenerateExpectedFileEntity()
        {
        ...
        [Fact, WebApiTrait(TraitGroup.All, TraitGroup.Generic)]
        public async Task GenericFileUpload_ValidGz_GenerateExpectedFileNode()
        {
            var blobInfo = await BlobStorage.GetFileInfo(BlobId, JohnId.ToString());
            blobInfo.Should().NotBeNull();
        ...
        [Fact, WebApiTrait(TraitGroup.All, TraitGroup.Generic)]
        public async Task GenericFileUpload_ValidGz_GenerateExpectedRecordNodesOnlyEmpty()
        {
