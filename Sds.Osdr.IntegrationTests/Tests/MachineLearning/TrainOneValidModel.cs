...
namespace Sds.Osdr.IntegrationTests
{
    public class TrainOneValidModelFixture
    {
        ...
        {
            //SourceBlobId = harness.JohnBlobStorageClient.AddResource(harness.JohnId.ToString(), "combined lysomotrophic.sdf", new Dictionary<string, object>() { { "parentId", harness.JohnId }, { "case", "valid one model" } }).Result;

            //harness.WaitWhileRecordsFileProcessed(SourceBlobId);
