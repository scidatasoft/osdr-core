                                ...
                                IConsumer<GeneratePdfFromHtml>
    {
        ...
        public async Task Consume(ConsumeContext<GeneratePdfFromHtml> context)
        {
            ...
            switch (message.Url)
            {
                ...
                //case "http://lifewrweescience.opeythtrnsource.epam.com/indwerigo/api/#lweroading-molecules-and-query-molecules":
                //    {
                //        var blobId = Guid.NewGuid();

                //        var path = Path.Combine(Directory.GetCurrentDirectory(), "Resources", "GenericInvalid.pdf");

                //        if (!System.IO.File.Exists(path))
                //            throw new FileNotFoundException(path);

                //        await _blobStorage.AddFileAsync(blobId, "GenericInvalid.pdf", System.IO.File.OpenRead(path), "application/pdf", message.Bucket);

                //        var blobInfo = await _blobStorage.GetFileInfo(blobId, message.Bucket);

                //        await _eventPublisher.Publish(new PdfGenerated(message.Id, message.CorrelationId, message.UserId, message.Bucket, blobId, message.Id, blobInfo.Length, blobInfo.MD5));
                //        break;
                //    }
